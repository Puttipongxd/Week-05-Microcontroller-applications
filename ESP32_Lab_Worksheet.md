...
docker-compose up -d ใช้รันคอนเทนเนอร์แบบเบื้องหลัง
docker-compose exec esp32-dev bash เข้า shell ของคอนเทนเนอร์เพื่อสั่งงานภายใน
2. **ESP-IDF Tools**: เครื่องมือไหนจาก Lab4 ที่จะใช้ในการ build โปรแกรม ESP32?
ใช้คำสั่ง idf.py build จาก ESP-IDF เพื่อคอมไพล์โปรแกรมสำหรับ ESP32
3. **New Tools**: เครื่องมือใหม่ที่ติดตั้ง (tree, htop) ใช้ทำอะไร?
tree แสดงโครงสร้างโฟลเดอร์, htop ดูการใช้ CPU และหน่วยความจำแบบเรียลไทม์
4. **Architecture Focus**: การศึกษา ESP32 architecture แตกต่างจากการทำ arithmetic ใน Lab4 อย่างไร?
การศึกษา ESP32 เน้นโครงสร้างฮาร์ดแวร์และการประมวลผลของชิป
ส่วน Lab4 เน้นการคำนวณเชิงตรรกะและการทำงานของโปรแกรมในระดับซอฟต์แวร์
...
SRAM ใช้เก็บตัวแปรและข้อมูลขณะทำงาน (runtime)
Flash Memory ใช้เก็บโปรแกรมและข้อมูลถาวร (firmware)
2. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
โค้ดอยู่ใน Flash (0x40000000–0x40FFFFFF)

ตัวแปร global/stack อยู่ใน SRAM (0x3FFB0000–0x3FFFFFFF)

Peripheral registers อยู่ใน IO range (0x3F400000–0x3F4FFFFF)

3. **Memory Usage**: ESP32 มี memory ทั้งหมดเท่าไร และใช้ไปเท่าไร?
   ESP32 มี SRAM ประมาณ 520 KB และ Flash 4 MB (บางรุ่นมากกว่านี้)
โดยโปรแกรมทั่วไปใช้ SRAM ราว 60–70% และ Flash ประมาณ 1–2 MB
...
Cache Efficiency
เพราะ sequential access ใช้ประโยชน์จาก spatial locality ทำให้ cache ดึงข้อมูลต่อเนื่องได้เร็วกว่า

Memory Hierarchy
SRAM อยู่ในชิป เข้าถึงเร็วกว่า ส่วน external memory ช้ากว่าเพราะต้องผ่านบัสภายนอก
 Stride Patterns
stride ใหญ่เกินไปทำให้ cache miss บ่อย ประสิทธิภาพลดลง
---
...
Core Specialization
Core 0 เหมาะกับงานระบบและสื่อสาร, Core 1 เหมาะกับงานประมวลผลหลัก
Communication Overhead
มี overhead เล็กน้อยจากการส่งข้อมูลระหว่าง core ผ่าน queue หรือ shared memory

Load Balancing
กระจายงานได้ดีเมื่อกำหนดหน้าที่ชัดเจน แต่ถ้างานไม่สมดุล core หนึ่งอาจทำงานหนักกว่าอีก core
...
**ข้อมูลนักศึกษา:**
- ชื่อ: ภัทรพล ปัญญาเครือ
- รหัสนักศึกษา: 67030172
- วันที่ทำการทดลอง: 29/10/2568
...
2. สิ่งที่เรียนรู้เพิ่มเติมเกี่ยวกับ ESP32 architecture:
  รู้โครงสร้าง Dual-core, การเชื่อมต่อ Wi-Fi/Bluetooth และการประหยัดพลังงาน

3. ความท้าทายที่พบในการทำ architecture analysis:
   ข้อมูลซับซ้อน ต้องทำความเข้าใจจาก datasheet และแผนผังระบบ
